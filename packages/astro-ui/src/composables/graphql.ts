...
export function useGraphQL<T extends RawGraphQLData>(query: string, variables: Record<string, unknown>): Promise<T>
export function useGraphQL<T extends RawGraphQLData>(
  query: string, variables?: Record<string, unknown>, params?: Partial<UseGraphQLParams>): Promise<T>
...
export async function useGraphQL<T extends RawGraphQLData>(
  ...
  const url = params?.url ?? import.meta.env.GRAPHQL_ENDPOINT
